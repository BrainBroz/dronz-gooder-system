...
| B-02 | Plataformas e contas externas | PARCIAL | entidades e providers de domínio existem; conexão e sincronização reais Amazon/eBay estão planejadas |
...
| B-17, B-35 | Venda e baixa patrimonial | FORA DO ESCOPO ATUAL | roadmap posterior às integrações, tracking e Financeiro |
...
| B-34 | Transporte por frete e tracking | PLANEJADO | será tratado depois de Amazon, eBay e sincronização de ordens |
...
- arquitetura de credenciais e operação dos conectores Amazon/eBay ainda deve ser contratada antes da implementação;
