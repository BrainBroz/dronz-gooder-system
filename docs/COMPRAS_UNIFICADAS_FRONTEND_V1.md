...
Implementação técnica do Batch 6, subordinada ao `AGENTS.md`, ao contrato `COMPRAS_UNIFICADAS_E_CHECKPOINTS_V1.md` e ao backend descrito em `COMPRAS_UNIFICADAS_BACKEND_V1.md`.
...
As queries de staging possuem namespace próprio. Detalhes usam o ID da compra; produtos e fornecedores usam `storeId`. Após uma mutação, overview, lista, detalhe e histórico relacionados são invalidados. A troca de loja em formulários limpa seleções incompatíveis.
...
O frontend transmite plataforma, conta, referência, IDs externos, Unicode e caixa conforme digitados. A normalização e deduplicação são responsabilidades do backend. O teste de regressão do frontend comprova apenas a preservação do payload; a dívida B5-A01 permanece no backend até tratamento próprio.
...
Integração real com providers, listagem de contas/merchants, matching automático, alterações no backend, Dashboard, Relatórios, UI-3C e qualquer mutação de estoque, logística ou financeiro.
