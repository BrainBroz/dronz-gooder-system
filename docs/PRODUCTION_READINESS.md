...
Referências `env:MARKETPLACE_*` não são credenciais: são ponteiros. Valores devem existir somente no ambiente/secret manager e nunca em banco, logs, respostas ou commits. Dronz e Gooder operam como compradores. Amazon SP-API e eBay Sell Fulfillment são seller-side e permanecem adiados. O próximo gate valida `GetMyeBayBuying`, consentimento/token, quota e resposta do keyset real. Para Amazon, e-mail autorizado é a fonte inicial planejada; a Amazon Business Reporting API permanece `PENDENTE_DE_ONBOARDING_EXTERNO` e será retomada futuramente. Gmail/Outlook exigirão OAuth, escopos mínimos, retenção e privacidade aprovados antes de qualquer ativação.
