...
| ----: | --------------------------------- | ---------------------------------------------------------------- | ------------------------------------------- | ------------------------------------------------------------------------- |
|     1 | Contrato Buyer Purchase Ingestion | definir contrato independente da fonte para compras realizadas   | staging e fundação de integração existentes | identidade, autorização, privacidade, idempotência e conflitos normativos |
|     2 | E-mail autorizado                 | ingerir mensagens relevantes sem ler conteúdo além do necessário | contrato buyer e OAuth/caixa aprovados      | correlação, deduplicação, retenção e auditoria                            |
|     3 | Documentos, invoices e CSV        | importar evidências e dados estruturados com preview             | contrato buyer e storage aprovado           | validação e importação idempotente                                        |
|     4 | Envios, pacotes e trackings       | consolidar atualizações posteriores de qualquer fonte            | fontes buyer implementadas                  | múltiplos envios/pacotes/códigos e histórico preservado                   |
|     5 | Tracking automático independente  | acompanhar códigos normalizados sem acoplamento à fonte          | envios e códigos consolidados               | eventos fora de ordem tratados e fallback manual                          |
|   6–8 | Financeiro, Vendas e Analytics    | evoluções funcionais posteriores                                 | contratos próprios e aprovação              | isolamento, auditoria e migrations incrementais                           |

### 37.1 Decisões oficiais de tracking

- O tracking automático depende das fontes autorizadas de buyer ingestion e da consolidação de envios/pacotes, não de uma API específica.
- O domínio de tracking é independente da fonte; adapters de e-mail, documento, CSV, manual ou marketplace traduzem dados para contrato canônico.
...
Gate documental: Batches 3–6, Auditoria Independente do Batch 7 e fundação técnica do Batch 8 aprovados. A próxima etapa oficial é o contrato normativo de Buyer Purchase Ingestion. Fontes automáticas e tracking continuam planejados, não implementados.
